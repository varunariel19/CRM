...
import { TeamAttachmentType, TeamConversation, TeamMessage, TeamUser } from '../core/types/teams.type';
...
import { TaskStatus } from './task-management.service';
...
export class TeamsService {
  ...
  private taskStatusHandlers: Array<(taskId: string, status: TaskStatus) => void> = [];
  ...
  async connect(handlers: TeamsConnectHandlers = {}): Promise<() => void> {
    const {
      ...
      onLeadStatusChanged, onLeadConverted, onNotification, onTaskStatusChanged
    } = handlers;
    ...
    if (onTaskStatusChanged) this.taskStatusHandlers.push(onTaskStatusChanged);
    ...
    return () => {
      ...
      if (onTaskStatusChanged) this.taskStatusHandlers = this.taskStatusHandlers.filter(h => h !== onTaskStatusChanged);
    };
  }
  ...
  private registerHandlers(): void {
    ...
    this.connection.on("TaskConverted", (taskId: string, status: TaskStatus) => {
      this.taskStatusHandlers.forEach(handler => handler(taskId, status));
    })
  }
}
